...
setupFunc () {
    ...
    select opt in "${options[@]}"; do
        # Convert menu response to index (associative array not used for Mac compatibility)
        local index="$((REPLY-1))"
...
startCheck () {
    # Check if a compose file has already been defined, if not: setup, else start/restart
